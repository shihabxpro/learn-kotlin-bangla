...
import { ArrowLeft, ArrowRight, ChevronRight, CheckCircle2, Trophy, Clock, BookOpen } from "lucide-react";
...
export default function ModulePage() {
  ...
  // Consistent Kitty Theme Accent for this milestone
  const ac = KITTY_ACCENTS[module.id] || module.accent;
  const kittyPersona = KITTY_CHAPTER_TITLES[module.id] || `${module.title} গাইড`;
  const prevModule = modules[module.id - 2];
  const nextModule = modules[module.id];
  const nextAc = nextModule ? (KITTY_ACCENTS[nextModule.id] || nextModule.accent) : ac;

  return (
    <div className="relative z-10 mx-auto w-full max-w-2xl px-4 pb-28 pt-28 sm:px-6 md:pt-32">
      {/* Top accent glow matching this milestone's Kitty color */}
      <div
        aria-hidden
        className="pointer-events-none absolute left-1/2 top-16 -z-10 h-[340px] w-[560px] -translate-x-1/2 rounded-full blur-[130px] opacity-70"
        style={{
          background: `radial-gradient(circle, ${ac}33 0%, rgba(255,117,143,0.15) 50%, transparent 80%)`,
        }}
      />
      ...
      <motion.header
        ...
        className="mt-8"
      >
        {/* Breadcrumb with Kitty branding */}
        <motion.nav
          variants={rise}
        ...
        {/* Milestone Mascot & Chapter Badge */}
        <motion.div variants={rise} className="mt-6 flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <p
                className="font-bengali text-[13px] font-bold tracking-[0.03em]"
                style={{ color: ac }}
              >
                ধাপ {bnDigits(module.id)} / {bnDigits(modules.length)}
              </p>
            </div>
            <p className="mt-1 font-bengali text-[12.5px] text-pink-200/60 flex items-center gap-1.5">
              <span>{kittyPersona}</span>
              <span>•</span>
        ...
        >
          <span className="flex items-center gap-1.5 rounded-full border border-pink-400/20 bg-pink-500/5 px-3 py-1">
            ...
            {bnDigits(module.lessons.length)} টি লেসন
          </span>
          <span className="flex items-center gap-1.5 rounded-full border border-pink-400/20 bg-pink-500/5 px-3 py-1">
        ...
        {/* Module 1 Special Kitty Milestone Completed Banner */}
      ...
      >
        {module.lessons.map((lesson, i) => (
          ...
          >
            {/* Number rail with milestone Kitty color */}
            <div className="flex flex-col items-center">
              <span
                className="grid size-11 flex-none place-items-center rounded-xl border font-bengali text-sm font-bold shadow-sm transition-transform duration-200 group-hover:scale-105"
                style={{
                  borderColor: `${ac}45`,
                  color: ac,
                  background: `linear-gradient(135deg, ${ac}22, #140b1c 70%)`,
                  boxShadow: `0 4px 16px -4px ${ac}35`,
                }}
              >
                {bnDigits(lesson.id)}
              </span>
              {i < module.lessons.length - 1 && (
                <span
                  ...
                  style={{
                    background: `linear-gradient(to bottom, ${ac}55, ${ac}12)`,
                  }}
                />
              )}
            </div>
          ...
          <div className="relative z-10 flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            <div className="space-y-2.5">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bengali text-[13px] font-bold" style={{ color: ac }}>
                  Kitty ব্যাজ
                </span>
              </div>
              <h3 className="font-display text-xl font-bold text-white sm:text-2xl">
                অভিনন্দন! {module.title} শুরু করুন
              </h3>
              <p className="max-w-xl font-bengali text-[14.5px] leading-relaxed text-pink-100/80">
                প্রতিটি লেসনে রয়েছে বিস্তারিত ব্যাখ্যা, কোড উদাহরণ এবং ইন্টারঅ্যাক্টিভ অনুশীলন।
              </p>
            ...
            <div className="flex-none">
              <Link
                ...
                className="group inline-flex items-center gap-3 rounded-2xl px-5 py-3.5 font-display text-[14.5px] font-bold text-white transition-all hover:scale-[1.03] hover:shadow-[0_0_40px_rgba(255,117,143,0.6)]"
              >
                <span className="text-base">🚀</span>
                <span className="font-bengali">পরবর্তী: {nextModule.title}</span>
      ...
      >
        {prevModule ? (
          <motion.div variants={rise}>
            ...
            >
              <div className="flex size-10 flex-none items-center justify-center rounded-xl border border-pink-400/20 bg-pink-500/10">
                <MilestoneCatIcon moduleId={prevModule.id} size={22} color={KITTY_ACCENTS[prevModule.id]} />
              </div>
              <span className="min-w-0">
                <span className="block font-bengali text-[13px] font-semibold text-pink-200">
                  পূর্ববর্তী মাইলস্টোন
                </span>
        ...
        {nextModule ? (
          <motion.div variants={rise}>
            ...
            >
              ...
              >
                <MilestoneCatIcon moduleId={nextModule.id} size={22} color={nextAc} />
              </div>
              <span className="min-w-0">
